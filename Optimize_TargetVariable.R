################################################### WARNING ! #################################################################
######## CODE WILL TAKE SOME TIME TO RUN AS IT IS TRYING TO FIND THE OPTIMAL EXPECTED DONATION WITHIN A RANGE #################
################################ PLEASE ALSO LET US KNOW IF THERE IS A WAY TO OPTIMIZE. THANKS! ###############################
...
################################################################################################################################ 
################################################### DEFINING PROFIT AS FUNCTION OF OPTIMIZATION ################################  
################################################### FOR LOGISTIC MODEL WITH STEPWISE FEATURE SELECTION##########################  


profit <- function(Expected_Donation) {

#Print Expected_Donation to track current value assigned to expected donation used during the optimization process

print(paste0("Current trial value of optimum expected_donation: ", Expected_Donation))
  
      
#Reads the raw data  

donor = read.table("C:/Users/yloh/Desktop/Predictive analytics/Group Project/data-20171115/donors.csv",sep=";",header = TRUE)  
train = read.table("C:/Users/yloh/Desktop/Predictive analytics/Group Project/data-20171115/campaign20130411.csv",sep=";",header = TRUE)  
test = read.table("C:/Users/yloh/Desktop/Predictive analytics/Group Project/data-20171115/campaign20140115.csv",sep=";",header = TRUE)  
gifts = read.table("C:/Users/yloh/Desktop/Predictive analytics/Group Project/data-20171115/gifts.csv",sep=";",header = TRUE)  
...
##########Deal with NA Values
#Omit the NA values in zipcode, d_zipcode, gender, language for train dataset
base_train <- subset(base_train, zipcode != 0) 

base_train$d_zipcode <- na.omit(base_train$d_zipcode) 
base_train <- subset(base_train, d_zipcode != 0) 

base_train$gender <- na.omit(base_train$gender) 
base_train <- subset(base_train, gender != 0) 

base_train$language <- na.omit(base_train$language) 
base_train <- subset(base_train, language != 0) 

#Omit the NA values in zipcode, d_zipcode, gender, language for test dataset
base_test <- subset(base_test, zipcode != 0) 

base_test$d_zipcode <- na.omit(base_test$d_zipcode) 
base_test <- subset (base_test, d_zipcode != 0) 

base_test$gender <- na.omit(base_test$gender) 
base_test <- subset (base_test, gender != 0) 

base_test$language <- na.omit(base_test$language) 
base_test <- subset (base_test, language != 0) 
...
################## Find the optimal proportion of population to target to obtain maximum profit ########################


# First we bind the test predictions with the test data
data_test_bind = cbind(base_test_unique,step_lrm_predict_test_opt)
# Then we sort this data according to the prediction:
...
data_test_bind$maxprofit = cumsum(data_test_bind$step_lrm_predict_test_opt*(Expected_Donation - Cost_Per_Letter) - (1 - data_test_bind$step_lrm_predict_test_opt)*0.5)

max_profit = max(data_test_bind$maxprofit)
return(max_profit)
}


################################################### OPTIMIZATION ##################################################### 
optimum_donation = optimize(profit, lower = 25, upper = 45, maximum = TRUE,
         tol = 1)
